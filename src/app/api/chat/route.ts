...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.error('GEMINI_API_KEY not configured, using fallback response');
      ...
      const stream = new ReadableStream({
        start(controller) {
          const textChunk = `data: ${JSON.stringify({
            text:
              "Hi! The AI assistant isn't fully configured yet, but you can still explore the portfolio. To enable it, set GEMINI_API_KEY in your environment.",
          })}\n\n`;
